...
import { blueDark, blueLight, generalStyleBoxModal, homeStyle, redWarn, tableStyleBoxModal } from "../../Styles/general";
import { colorText } from "../../Styles/general";

import { setAvatarProps } from "../../hooks/setAvatarProps";
import { getAllCotization, getCotizationsOverview, getCotizationsPerAgent, removeCotization } from "../../services/cotizationService";
...
type AgentCotizationsProps = {
    open: boolean;
    handleOpen: (boolean) => void;
    agentId: string;
}

const AgentCotizations: React.FC<AgentCotizationsProps> = ({ open = false, handleOpen, agentId }) => {
    ...
    const { data: sourceData, isSuccess, isFetching, refetch,  } = useQuery<cotizationGeneralData[]>({ queryKey: ["getCotizationsPerAgent"], queryFn: () => getCotizationsPerAgent(agentId), refetchOnWindowFocus: false });
    ...
    useEffect(()=>{
        if(open ){
            ...
            queryClient.removeQueries({
                queryKey: ["getCotizationsPerAgent"]
            });
        }
    },[open])
...
export default AgentCotizations;