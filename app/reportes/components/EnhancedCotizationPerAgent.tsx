...
import { blueDark, blueLight, generalStyleBoxModal, homeStyle, redWarn } from "../../Styles/general";
...
import AgentCotizations from "./AgentCotizations";

const EnhancedCotizationsPerAgents = () =>{
  const [warnMessageOpen, setWarnMessageOpen] = useState(false);
  const [viewCotizations, setViewCotizations] = useState(false);
  ...
  const handleOpenCotizations = (id: string) => {
    setViewCotizations(true);
    setAgentId(id);
  }
    ...
    return [
          {
            ...
            getActions: ({ id }) => {
                return [
                    <GridActionsCellItem
                        key={2}
                        icon={<VisibilityIcon sx={{color: blueLight}}/>}
                        label= {"Ver"}
                        color="primary"
                        onClick = {() => handleOpenCotizations(id)}
                    />,
                ]
            },
  ...
  return (
    <>
      ...
      <AgentCotizations
        open={viewCotizations}
        handleOpen={setViewCotizations}
        agentId={agentId}
      />
    </>
  )
...
export default EnhancedCotizationsPerAgents;